...
class LiceoForm(forms.ModelForm):
    ...
    def clean_cedula(self):
        ...
        if Liceo.objects.filter(cedula=cedula):
            raise forms.ValidationError(_("La Persona ya se encuentra registrada"))
        return cedula

    """def clean(self):
        cleaned_data = super(LiceoForm, self).clean()
    ...
    def clean_correo(self):
        ...
        if Liceo.objects.filter(correo=correo):
    ...
    def __init__(self, *args, **kwargs):
        super(LiceoForm, self).__init__(*args, **kwargs)
